...
export function BellIcon({ userId }) {
  ...
  function handleItemClick(n) {
    ...
    setOpen(false);
    if (n.type === "new_prompt_from_followed" || n.type === "new_prompt") {
      navigate({
        to: "/public/$promptId",
        params: { promptId: n.payload.prompt_id },
      });
    } else if (n.type === "prompt_upvoted") {
      navigate({
  ...
  function getMessage(n) {
    if (n.type === "new_prompt_from_followed" || n.type === "new_prompt")
      return `${n.payload.author_name || "A creator you follow"} published "${n.payload.prompt_title || n.payload.title || "Untitled"}"`;
    if (n.type === "prompt_upvoted")
      return `${n.payload.author_name || "Someone"} upvoted "${n.payload.prompt_title || "Untitled"}"`;
    if (n.type === "new_follower") return `Someone started following you!`;
            ...
            {isLoading ? (
              ...
              notifications.map((n) => {
                ...
                return (
                  ...
                  >
                    <span
                      className={`text-sm leading-tight ${isUnread ? "font-bold text-black" : "font-medium text-slate-700"}`}
                    >
                      {getMessage(n)}
                    </span>
